...
func ToKindProps[T KindProperties](v cue.Value) (Def[T], error) {
	_, props, err := getProps[T](v)
	return Def[T]{
		V:          v,
		Properties: props,
	}, err
...
func ToDef[T KindProperties](v cue.Value) (Def[T], error) {
	unified, props, err := getProps[T](v)
	return Def[T]{
		V:          unified,
		Properties: props,
	}, err
...
func getProps[T KindProperties](v cue.Value) (cue.Value, T, error) {
	props := new(T)
	if !v.Exists() {
		return cue.Value{}, *props, ErrValueNotExist
	}
	...
	item := v.Unify(kdef)
	if item.Err() != nil {
		return cue.Value{}, *props, errors.Wrap(errors.Promote(ErrValueNotAKind, ""), item.Err())
	}

	if err := item.Decode(props); err != nil {
		// Should only be reachable if CUE and Go framework types have diverged
		return cue.Value{}, *props, errors.New(errors.Details(err, nil))
	}
