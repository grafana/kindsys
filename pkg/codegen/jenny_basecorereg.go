...
// (e.g. pkg/kind).
func BaseCoreRegistryJenny(path string, kindrelroot string, kindPackagePrefix string) ManyToOne {
	return &genBaseRegistry{
		path:              path,
		kindrelroot:       kindrelroot,
		kindPackagePrefix: kindPackagePrefix,
	}
}
...
func (gen *genBaseRegistry) Generate(kinds ...kindsys.Kind) (*codejen.File, error) {
	...
	if err := tmpls.Lookup("kind_registry.tmpl").Execute(buf, tvars_kind_registry{
		...
		KindPackagePrefix: filepath.ToSlash(filepath.Join(gen.kindPackagePrefix, gen.kindrelroot)),
		Kinds:             cores,
	}); err != nil {
